...
def test_wait_for_dock_area_pumps_events_before_final_match_check(monkeypatch) -> None:
    calls: list[str] = []
    ...
    monkeypatch.setattr(
        "tests.test_config_controller._dock_area_matches",
        lambda _main_window, _dock, _target_area: calls.append("match") or next(matches),
    )

    _wait_for_dock_area(object(), object(), object(), object(), attempts=1)

    assert calls == ["pump:5", "match", "pump:5", "match"]
